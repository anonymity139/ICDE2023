             ...
             int &Qcount, int ROUND, double selectRatio, int &TID, ofstream &fp)
{
    ...
    while(t_count > 1)
    {
        ...
        {
            if(option == 1) //score1 >= score2)//update the relational graph
            {
    ...
    std::cout << "\n-----------------------------------------------------------------------\n";
    printf("%33s: %5d \n", "The number of questions asked", Qcount);
    std::cout << "-----------------------------------------------------------------------\n";
    //out_cp << Qcount << "    " << time_cost << "\n";
    car_set->tuples[c->clusters[0]->tuples[0]->id]->final_result("General", Qcount, 1, fp);
